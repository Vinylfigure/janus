...
5. Untouchable: any issue failing the consumption gate. The gating labels are `question:`, `loop:hold`, `inbox:`, and `human-check:` — that list lives, executable, in `scripts/check-ready.sh` (Human Attention Protocol v1, docs/ATTENTION.md), because prose that "generally understood" the labels consumed an unanswered `question:` and built a branch the operator had never chosen (#42). The kill switch is pausing the routine.
...
1. Ready sweep: consult the Status dashboard issue and the open PRs first — an unmerged `claude/` PR from a prior firing outranks starting new work when its checks are red. Then list open `task:` issues. Ready = labeled `task:`, carries a done-means, is within this environment's tool grant, carries none of the gating labels (hold-in-mind 5), and references no open `question:`. Before committing to a candidate, run `scripts/check-ready.sh --body <body-file> <its labels...>` — exit 0 is the only proceed; the reason it prints is the reason to record when skipping. Take the oldest ready task unless one is explicitly marked priority.
...
2. Consume (exactly one): run the normal delivery path — `/plan-feature` for non-trivial work (its own rule lets simple tasks skip ceremony) → implement → `/verify-loop` → the `verifier` agent's judgment → `/ship` as a PR whose body says `Closes #N`. Descoped remainders follow the descope gate: new `task:` issues with `discovered-from:` refs. When the task carries a Human check section, apply `human-check:` to the issue at delivery and say so in the PR body — machine verification green is not done; the operator's check is the next actor, and the merge waits for it.
3. Idle generation (only when NO task is ready): triage first — for each open `inbox:` item, draft a done-means from repo/product context and file it as a `task:` proposal carrying `discovered-from: inbox #N` (close the inbox item with a pointer), or convert it to a `question:` when a product decision is needed; a thought the repo cannot yet act on stays in the inbox untouched. Then, with triage room left, evaluate the repo against its stated purpose and objectives, the ledger's candidates and efficacy notes, and recent merged PRs; where the environment allows, add a bounded research pass on the repo's domain. File at most 2 new issues total across both arms, each proposal with a done-means and a `discovered-from:` ref. Then stop — the next firing executes.
