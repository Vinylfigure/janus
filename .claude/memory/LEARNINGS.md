...
- Trigger: methodology review adopted the critique that nothing measures whether promoted rules reduce failures; the full baseline-vs-scaffold evaluation was deliberately deferred — the template has no real coding tasks to measure (origin: user-supplied critique, user decision 2026-07-24). Re-open trigger: the first bootstrapped child with real task history. Protocol sketch preserved: fixed task set; arms = plain Claude Code / verify-only / memory-only / full scaffold; measure task success, repeated-error rate, interventions, tokens; test longitudinally (task N benefits from lessons of tasks 1..N-1). observed: 2026-08-16 — re-open trigger satisfied: aegis-sentinel is the first bootstrapped child with real task history (13 merged PRs), and its memory audit found the session loop never ran there (zero /reflect//evolve//recalibrate in 3 weeks; see L-039 for the cause); first interim data point is negative — the loop did not survive replication, so the baseline comparison now has a live subject. observed: 2026-08-25 — the comparison RAN (docs/L037-AEGIS-COMPARISON.md, closes #20): aegis-sentinel loop-dead 07-27→08-16 vs loop-active 08-16→08-25. Verdict: supports, narrowly — work evaporation ended (a stranded walking-skeleton build + an Evidence-3 lesson vanished silently in the dead period vs 12 trail-carrying task: issues, 10 consumed, in the active one) and repeat-mistake latency collapsed (~75 min self-caught ID collision vs a 3-week undetected loop death), but prose-tier rules kept being skipped on autonomous firings until mechanized, operator-attention confounds bar throughput attribution, and the silent 3-week death stands as negative evidence on robustness. Refined claim: the loop works exactly as far up the enforcement ladder as it pushes its lessons
...
## L-042 · 2026-08-16 · A merged milestone is a trigger, not a stopping point — surface the next task mechanically
- Trigger: user correction 2026-08-16 — after two merged milestones the session reported "say the word" while docs/EXECUTION-PLAN.md named the next unblocked task; "I shouldn't have to tell you when I want it … shouldn't the loop harness do that?"; the Conduct-don't-wait directive existed as prose with no mechanism carrying it (origin: user correction)
...
- Trigger: the descope loop landed capture (part B → `task:` issue) with no consumer — the operator asked why deferred tasks aren't looped to continue and why an empty backlog doesn't trigger evaluation of what should exist next; loop-engineering research confirmed the shape (developersdigest.tech/blog/loop-engineering-definitive-guide: "a loop with no verifier produces wrong answers faster", the worker never grades its own homework; Huntley's Ralph loop: fresh context per iteration, state on disk, one task per pass) (origin: user correction + fetched content, verified in main thread, 2026-08-16). Second instance, same day: the rule was promoted into /work-loop and merged (janus#16) with **no Routine armed and §6's fuel unfiled** — the skill existed, the schedule didn't — and the intent-ide conductor session reproduced it by ending its turn offering run-now-vs-Routine instead of arming; the operator had to ask why the end state isn't happening (origin: user correction, 2026-08-16; refs: janus#16, docs/AUDIT-2026-08.md §6, intent-ide Flow v1 session)
- Rule: a backlog without a scheduled consumer is a parking lot — pair every capture channel with a routine that consumes exactly one ready item per firing and delivers by PR, generates proposals only when idle, and never executes a proposal in the firing that created it. The pairing is part of the loop's *delivery*: a firing schedule that exists only as a skill is prose — the same session arms the routine, or files the arming itself as a `task:` issue naming the exact call (approval gates make the issue the floor, never silence)
- Scope: portable
- Evidence: 2
...
## L-052 · 2026-07-27 · Restraint rules bind the template's machinery, not the user's product ask
- Trigger: user correction — a docs-only plan deferred a requested multi-repo conductor on L-014 grounds and was rejected ("regardless of L-014, is there anything I can do?"); the capability was then built as a new specialized child repo with the template untouched, satisfying both the ask and the rule (janus multi-repo orchestration session; authored as L-039 on stranded branch claude/janice-multi-repo-orchestration-3h96bg, renumbered at 2026-08-16 salvage — main had since assigned L-039)
