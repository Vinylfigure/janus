...
- Template plumbing is wired: `scripts/verify.sh quick|full` (full runs the loop-manifest check `scripts/check-loops.sh` then the hook fixture suite `scripts/test-hooks.sh`; CI runs `verify.sh full` on every PR).
<!-- janus:facts:end -->
